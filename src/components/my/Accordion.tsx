...
const Accordion: FC<AccordionProps> = ({ data, roll, id }) => {
  ...
  const statusRender: () => JSX.Element | undefined = () => {
    if (roll === '판매자') {
      if (data.transactionStatus === '거래신청') {
        return (
          <File
      ...
      } else if (data.transactionStatus === '계약검토') {
        return (
          <>
      ...
      } else if (data.transactionStatus === '이체검토') {
        return <button onClick={clickPayMoney}>확인완료</button>
      }
    } else if (roll === '구매자') {
      if (data.transactionStatus === '거래승인') {
        return (
          <>
            <File
              transactionMutate={transactionMutate}
      ...
      } else if (data.transactionStatus === '이체대기') {
        return (
          <>
